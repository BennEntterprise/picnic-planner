...
import type { DailyForecastList, HistoricalDataMap } from './types';

// Generate mock forecast data for the next 14 days
export const mockForecastData: DailyForecastList = Array.from({
  length: 14,
}).map((_, index) => {
  ...
  return {
    ...
    precipitation_probability_max: isRainy
      ? 60 + Math.floor(Math.random() * 40)
      : Math.floor(Math.random() * 20),
    windspeed_10m_max: isWindy
      ? 25 + Math.random() * 15
      : 5 + Math.random() * 15,
    windgusts_10m_max: isWindy
      ? 35 + Math.random() * 20
      : 10 + Math.random() * 15,
    winddirection_10m_dominant: Math.floor(Math.random() * 360),
    uv_index_max: isHot ? 7 + Math.random() * 4 : 3 + Math.random() * 4,
    relativehumidity_2m_max: isRainy
      ? 80 + Math.random() * 15
      : 50 + Math.random() * 20,
...
export const mockHistoricalData: HistoricalDataMap = {};
